...
const extractValue = (value: string | { normalizedValue: string } | any): string => {
...
export default function PatientDashboard() {
  ...
  const { updateStatusOptimistically, isUpdating: isUpdatingAppointmentStatus } = useOptimisticAppointmentStatusUpdate();
  ...
  if (patientsError || patientError) {
    const errorMessage = patientsError?.message || patientError?.message || "Unknown error occurred";
    return (
      <div className="min-h-screen bg-gray-50">
        <div className="p-6">
          <div className="bg-white border border-red-200 rounded-sm p-4 max-w-md">
            <div className="flex items-center gap-2 text-red-600 mb-2">
              ...
              <span className="text-sm font-medium">Failed to load patient information</span>
            </div>
          ...
          <div className="col-span-8 space-y-6">
                ...
                {upcomingAppointments.length === 0 ? (
                  ...
                  <div className="divide-y divide-gray-100">
                    {upcomingAppointments.slice(0, 3).map((appointment) => {
                      const doctorName = ('doctor' in appointment && appointment.doctor)
            ...
            <section>
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-xs font-semibold text-gray-600 uppercase tracking-wider">
                  Medical Documents
                  <span className="ml-2 font-mono text-gray-400 font-normal">{documents.length}</span>
                </h2>
                <button className="text-xs font-medium text-gray-700 hover:text-gray-900">
                  Request Document
                </button>
              ...
              <div className="bg-white border border-gray-200 rounded-sm">
                {isDocumentsLoading ? (
                  ...
                  <div className="px-4 py-8 text-center">
                    ...
                    <p className="text-sm text-gray-600 mb-1">No documents available</p>
                    <p className="text-xs text-gray-500">Medical records will appear here</p>
                  </div>
                ) : (
                  <div className="divide-y divide-gray-100">
                    {documents.map((doc) => (
                      <div key={doc.id} className="px-4 py-3 hover:bg-gray-50 transition-colors">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            <span className={`px-2 py-1 text-xs font-mono border rounded-xs ${getDocumentTypeStyles(doc.type)}`}>
                              {doc.type}
                            </span>
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-2 mt-0.5">
                                ...
                                <span className={`text-xs ${
                                  ...
                                  doc.status === 'ACTIVE' ? 'text-blue-600' :
                          ...
                          <button className="flex items-center gap-1 text-xs font-medium text-gray-700 hover:text-gray-900 disabled:opacity-50 disabled:cursor-not-allowed" disabled={doc.status === 'PENDING'}>
