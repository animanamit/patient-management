...
import { useTodayAppointments, useOptimisticAppointmentStatusUpdate } from "@/hooks/use-appointments";
import { DoctorId, AppointmentStatus, AppointmentWithDetails, Appointment } from "@/lib/api-types";
...
const StatsSkeleton = () => (
  <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
    {[1, 2, 3, 4].map((i) => (
      <Card key={i} className="border-deep-blue/15 bg-gradient-to-br from-deep-blue/3 to-sky/5">
        <CardContent className="p-6">
...
const AppointmentCard = ({ 
  ...
  const { updateStatusOptimistically, isUpdating } = useOptimisticAppointmentStatusUpdate();
  ...
  const patientId = ('patient' in appointment) 
        ...
        <CardContent className="p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-charcoal">{stats.totalToday}</p>
            </div>
...
export default function DoctorDashboard() {
  ...
  const { data: doctorData, isLoading: isDoctorLoading, error: doctorError } = useDoctor(firstDoctorId!, {
    enabled: !!firstDoctorId
  });
  
  // Fetch today's appointments for this doctor
  const { data: appointmentsData, isLoading: isAppointmentsLoading, error: appointmentsError } = useTodayAppointments(firstDoctorId!, {
    enabled: !!firstDoctorId
  });
  ...
  const handleAppointmentStatusUpdate = (appointmentId: string, newStatus: AppointmentStatus) => {
      ...
      <div className="min-h-screen bg-gradient-to-br from-white via-pale-blue/20 to-sky/30">
        ...
        <main className="container mx-auto px-4 py-8">
          <Alert variant="destructive" className="max-w-2xl mx-auto">
            ...
            <AlertDescription>
              Failed to load doctor information. Please try refreshing the page or contact support if the problem persists.
            </AlertDescription>
          </Alert>
        </main>
      </div>
        ...
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="text-right">
              ...
              <div className="text-xs text-muted-foreground">{doctor.specialization || 'General Practitioner'}</div>
            </div>
          </div>
        </div>
      </header>
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-6">
                ...
                <Card className="border-mint/30 bg-white/90">
                  <CardContent className="p-8 text-center">
                    ...
                    <h3 className="text-lg font-medium text-charcoal mb-2">
                      {searchQuery ? 'No matching appointments' : 'No appointments scheduled'}
                    </h3>
                    <p className="text-muted-foreground">
                      {searchQuery ? 'Try adjusting your search criteria' : 'Enjoy your free day!'}
                    </p>
                  </CardContent>
                </Card>
              ) : (
          ...
          <TabsContent value="patients" className="space-y-6">
            <div className="flex justify-between items-center">
              <h3 className="text-xl font-semibold text-charcoal">Patient Management</h3>
              <Button variant="outline" className="border-deep-blue text-deep-blue hover:bg-deep-blue hover:text-white">
                Search All Patients
              </Button>
            </div>
            ...
            <Card className="border-mint/30 bg-white/90">
              <CardContent className="p-8 text-center">
                ...
                <h3 className="text-lg font-medium text-charcoal mb-2">Patient Management</h3>
