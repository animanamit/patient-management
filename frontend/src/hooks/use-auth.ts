...
export const useAuth = () => {
  ...
  const signUpWithEmail = async ({
    ...
    try {
      ...
      if (
        ...
        errorMessage =
          "Authentication request blocked. The server may need to restart to apply configuration changes.";
      }
  ...
  const sendPhoneOTP = async (phoneNumber: string) => {
    ...
    try {
      ...
      toast.success(
        `Check your SMS for the verification code sent to ${formatPhoneDisplay(
          phoneNumber
        )}`,
        {
